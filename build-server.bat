...
echo Pulling jchristn77/switchboard:%VERSION_TAG% into the local image store...
docker pull jchristn77/switchboard:%VERSION_TAG%
if errorlevel 1 (
    echo Failed to pull jchristn77/switchboard:%VERSION_TAG% into the local image store.
    exit /b %errorlevel%
)
docker pull jchristn77/switchboard:latest
if errorlevel 1 (
    echo Failed to pull jchristn77/switchboard:latest into the local image store.
    exit /b %errorlevel%
)
